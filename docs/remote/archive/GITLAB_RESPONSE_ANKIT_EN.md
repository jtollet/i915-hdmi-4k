...
- The poll completed successfully just before the timeout limit

This suggests that:
1. The Cisco Desk Pro monitor needs almost the full 200ms timeout period to initialize scrambling
2. The polling mechanism works correctly as designed
3. Without the polling delay, the driver would continue too early, before the monitor is ready
...
Good point about sending to intel-xe@lists.freedesktop.org. Since the display code is shared between i915 and xe drivers, this fix will benefit both. I'll send the patch there as well.
